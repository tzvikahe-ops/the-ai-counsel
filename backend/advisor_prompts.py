...
CONSENSUS_TAG_INSTRUCTION_HEBREW = (
    "\n\nחשוב: סיים את תגובתך בסקשן \"אות הסכמה\", ואז בשורה אחרונה נפרדת "
    "כתוב בדיוק CONSENSUS_SCORE: N, כאשר N הוא מספר בודד בין 1 ל-5. "
    "השתמש ב-1 אם אתה לא מסכים לחלוטין עם עמדת הקבוצה המתגבשת, "
    "ב-3 אם אתה ניטרלי או לא מוחלט, וב-5 אם אתה מסכים לחלוטין ומוכן להתכנס. "
...
ADVISOR_ROUND1_PROMPT_HEBREW = (
    ...
    "כתוב ישיר ותמציתי. אם תחרוג מ-150 מילים, התגובה עלולה להיות מסומנת באזהרה."
    "{consensus_tag}"
)
...
ADVISOR_CROSS_POLLINATION_PROMPT_HEBREW = (
    ...
    "שמור על קצרנות. אל תכתוב מסות. השתמש בשמות היועצים כפי שהם מופיעים "
    "בטרנסקריפט (בעברית)."
)
...
ADVISOR_VERDICT_PROMPT_HEBREW = (
    ...
    "## חילוקי דעות\n"
    "עבור כל חילוקי דעות, צור שורה עם: נקודת המחלוקת, עמדת כל צד, ואיזה טיעון "
